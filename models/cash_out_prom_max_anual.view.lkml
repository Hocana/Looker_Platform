...
view: cash_out_prom_max_anual {
  derived_table: {
    sql: select MAX(Promedio_4semanas_Diario) Max_Diario
      ,MAX(Promedio_4semanas_Lunes) Max_Lunes
      ,MAX(Promedio_4semanas_SD) Max_SD
      ,MAX(Promedio_4semanas_Viernes) Max_Viernes
      ,Banco 
      from 
      (select T.Banco
            ,T.Anio
  ...
  dimension: max_diario {
    type: number
    sql: ${TABLE}.Max_Diario ;;
  }
  ...
  dimension: max_viernes {
    type: number
    sql: ${TABLE}.Max_Viernes ;;
  }
